...
class SpyDelegateSeenMessageList: ChatDelegates {
    ...
    func chatReady() {
        guard let _ = asyncExpectation else {
            XCTFail("SpyDelegateSeenMessageList was not setup correctly. Missing XCTExpectation reference")
            return
        }
...
class SeenMessageListTest: XCTestCase {
    ...
    //@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
    func test_seen_message_list_uniqueId() {
        ...
        let spyDelegate = SpyDelegateSeenMessageList()
        ...
        waitForExpectations(timeout: 9) { error in
            ...
            let myExpectation = self.expectation(description: "Message Seen List")
            let sendParams: JSON = ["threadId": 1327]
            
            self.myChatObject?.messageSeenList(params: sendParams, uniqueId: { (seenMessageListUniqueId) in
                log.debug("Message Seen List with params: (threadId: 1327) Unique Id Response: \n|| \(seenMessageListUniqueId)", context: "Test")
    ...
    // MARK: - test with params: [contactId: 563]
    //@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
    func test_seen_message_list_response() {
        myChatObject = Chat(socketAddress: socketAddress, ssoHost: ssoHost, platformHost: platformHost, fileServer: fileServer, serverName: serverName, token: token, typeCode: 1, msgPriority: 1, msgTTL: messageTtl, httpRequestTimeout: nil, actualTimingLog: nil, wsConnectionWaitTime: Double(wsConnectionWaitTime), connectionRetryInterval: connectionRetryInterval, connectionCheckTimeout: connectionCheckTimeout, messageTtl: messageTtl, reconnectOnClose: true)
        ...
        waitForExpectations(timeout: 9) { error in
            ...
            self.myChatObject?.messageSeenList(params: sendParams, uniqueId: { (seenMessageListUniqueId) in
                ...
                log.debug("Message Seen List with params: (threadId: 1327) Test Response: \n|| \(responseJSON)", context: "Test")
                self.somethingWithDelegateAsyncResult = true
                myExpectation.fulfill()
            })
