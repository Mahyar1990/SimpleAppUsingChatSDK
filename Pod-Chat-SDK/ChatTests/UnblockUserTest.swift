...
class SpyDelegateUnblockUser: ChatDelegates {
    ...
    func chatReady() {
        guard let _ = asyncExpectation else {
            XCTFail("SpyDelegateUnblockUser was not setup correctly. Missing XCTExpectation reference")
            return
        }
...
class UnblockUserTest: XCTestCase {
    ...
    //@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
    func test_Unblock_User_uniqueId() {
        ...
        let spyDelegate = SpyDelegateUnblockUser()
        ...
        waitForExpectations(timeout: 9) { error in
            ...
            let myExpectation = self.expectation(description: "Unblock Contact")
            let unblockUserParameters: JSON = ["blockId": 62]
            
            self.myChatObject?.unblockContact(params: unblockUserParameters, uniqueId: { (unblockUserUniqueId) in
                log.debug("Unblock User with params: (blockId: 41) Unique Id Response: \n|| \(unblockUserUniqueId)", context: "Test")
                self.somethingWithDelegateAsyncResult = true
                myExpectation.fulfill()
            }, completion: { (responseJSON) in
                log.debug("Unblock User with params: (blockId: 41) Test Response: \n|| \(responseJSON)", context: "Test")
            })
    ...
    // MARK: - test with params: [contactId: 563]
    //@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
    func test_Unblock_User_With_blockId41() {
        myChatObject = Chat(socketAddress: socketAddress, ssoHost: ssoHost, platformHost: platformHost, fileServer: fileServer, serverName: serverName, token: token, typeCode: 1, msgPriority: 1, msgTTL: messageTtl, httpRequestTimeout: nil, actualTimingLog: nil, wsConnectionWaitTime: Double(wsConnectionWaitTime), connectionRetryInterval: connectionRetryInterval, connectionCheckTimeout: connectionCheckTimeout, messageTtl: messageTtl, reconnectOnClose: true)
